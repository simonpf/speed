...
def resample_scalar(data: xr.DataArray) -> xr.DataArray:
    ...
    data_g = data_s.interp(
        latitude=GLOBAL.lats, longitude=GLOBAL.lons, method="nearest"
    )
    return data_g
...
def load_mrms_data(granule: Granule) -> Union[xr.Dataset, None]:
    ...
    ro_filename = (
        precip_1h_gc_rec.filename.replace("MultiSensor", "RadarOnly")
        .replace("GaugeCorr", "RadarOnly")
        .replace("_Pass2", "")
    )
    precip_1h_ro_rec = [rec for rec in precip_1h_ro_recs if rec.filename == ro_filename]
    ...
    precip_1h_ro_data = mrms.precip_1h.open(precip_1h_ro_rec).drop_vars(
        ["time", "valid_time"], errors="ignore"
    )
    precip_1h_gc_data = mrms.precip_1h_gc.open(precip_1h_gc_rec).drop_vars(
        ["time", "valid_time"], errors="ignore"
    )
...
def downsample_mrms_data(
    mrms_data: xr.Dataset, grid: Optional[AreaDefinition] = None
) -> xr.Dataset:
    """
    ...
    smoothed = xr.Dataset(
        {
            ...
            "surface_precip": (
                ("latitude", "longitude"),
                surface_precip.astype(np.float32),
            ),
            "surface_precip_nn": (
                ("latitude", "longitude"),
                mrms_data.surface_precip.data.astype(np.float32),
            ),
            "gauge_correction_factor": (
                ("latitude", "longitude"),
                gauge_correction_factor.astype(np.float32),
            ),
            "gauge_correction_factor_nn": (
                ("latitude", "longitude"),
            ...
            "time": (
                ("latitude", "longitude"),
                mrms_data.time.data.astype("datetime64[ns]"),
            ),
        }
    )
...
class MRMS(ReferenceData):
    ...
    def load_reference_data(
        ...
        for granule in list(granules):
            ...
            if col_start is None:
                ...
                lons_input = lons_input[
                    (lons_input >= lons.min()) * (lons_input <= lons.max())
                ]
                lats_input = lats_input[
                    (lats_input >= lats.min()) * (lats_input <= lats.max())
                ]
        ...
        if len(mrms_data) == 0:
            LOGGER.warning(
                "Unable to load complete MRMS data for input graule %s.", input_granule
            )
            return None
        ...
        LOGGER.info("Downsampling MRMS data for input granule %s", input_granule)
        ...
        latitudes = (
            input_data.latitude if "latitude" in input_data else input_data.latitude_s1
        )
        longitudes = (
            input_data.longitude
            if "longitude" in input_data
            else input_data.longitude_s1
        )
        ...
        LOGGER.info(
            "Calculating footprint averages for input granule %s", input_granule
        )
